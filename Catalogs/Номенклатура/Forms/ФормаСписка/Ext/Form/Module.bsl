	...
	Если ЭтоПодбор Тогда
		...
		Если Параметры.Свойство("ТипВладельца") Тогда
			...
			Если Не РаботаСЗакупками И РезервированиеЗапасов Тогда
				ВариантыЗаполненияКолонкиРезерв = ПолучитьЗначениеНастройкиПользователя(Пользователи.ТекущийПользователь(), "ЗаполнятьКолонкуРезервПриДобавленииВКорзину");
...
Функция ПолучитьКоэффициентЕдиницы(ЕдиницаИзмерения)
...
Функция ПолучитьЗначениеНастройкиПользователя(Пользователь, Настройка, ПустоеЗначение = Неопределено)
...
Функция ЗаполнениеКолонкиРезерв(ТипДокументаВладельца, ВидОперации, ВариантыЗаполненияКолонкиРезерв)
	...
	Если Не ЗначениеЗаполнено(ВариантыЗаполненияКолонкиРезерв) 
		...
		Если ТипДокументаВладельца = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
			Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу")
				Или ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаряд") Тогда
				Возврат Истина
			КонецЕсли;
		КонецЕсли;
...
Процедура ПолученШтрихкод(ДанныеШтрихкода) Экспорт
	...
	Если Не МассивДанных = Неопределено И МассивДанных.Количество() Тогда
		...
		Если МассивДанных.Количество() > 1 Тогда
			...
			ОткрытьФорму("Справочник.Номенклатура.Форма.ДублиНоменклатурыПоШтрихКоду", ПараметрыОткрытия, ЭтаФорма,,,,
	...
	Иначе
		...
		Если ИспользуютсяХарактеристикиНоменклатуры Тогда
			...
			Пока РезультатЗапроса.Следующий() Цикл
				...
				НоваяСтрока.СтавкаНДСПартии = НоваяСтрока.Партия.СтавкаНДС;
	...
	ИначеЕсли ИспользуютсяХарактеристикиНоменклатуры И ИспользуютсяПартииНоменклатуры Тогда
		...
		Пока ДеревоРезультат.Следующий() Цикл
			...
			Пока ВыборкаПоПартиям.Следующий() Цикл
				...
				СтрокаПоПартии.СтавкаНДСПартии = СтрокаПоПартии.Партия.СтавкаНДС;
				...
				Пока ВыборкаПоСкладам.Следующий() Цикл
					...
					СтрокаПоСкладу.СтавкаНДСПартии = СтрокаПоСкладу.Партия.СтавкаНДС;
					СтрокаПоСкладу.ПредставлениеПоказателя = Строка(СтрокаПоСкладу.Показатель) + СтрокаОтправительИлиПолучатель(СтрокаПоСкладу.Склад);
...
Процедура ВывестиУровеньЯчеек(ВыборкаРодитель, Ссылка, СтрокаРодитель, ПроверятьВывод, СтруктураПоиска
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаРодитель.ЕстьЯчейка) И ВыводитьУровнень Тогда
			...
			СтрокаДерева.СтавкаНДСПартии = СтрокаДерева.Партия.СтавкаНДС;
