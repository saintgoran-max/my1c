...
Процедура ОпределитьНомерПриказа()
	...
	ГодКлассификатораМакс = Год(Макс(Справочники.ВидыНалогов.ДатаАктуальностиКлассификатора(),
		Справочники.ВидыНалогов.ДатаВторогоКлассификатора()));
	
	Если Период >= ГодКлассификатораМакс Тогда
		// Указывается новый классификатор
		ИмяМакета      = Справочники.ВидыНалогов.ИмяМакетаКБКНовый();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 10 июня 2024 г. № 85н
			|(в редакции приказа Минфина от 13.11.2024 № 165н)'");
	Иначе
		// Указывается текущий, или старый классификатор
		ИмяМакета      = Справочники.ВидыНалогов.ИмяМакетаКБКБезГода();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 1 июня 2023 г. № 80н
			|(в редакции приказа Минфина от 19.09.2024 № 130н)'");
	КонецЕсли;
