...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан) Экспорт
	...
	Если КлассификаторКТРУ.ЗагружатьКлассификаторПолностью() Тогда
...
Функция НеразделенныйКлассификатор() Экспорт
	...
	ИмяОбщегоРеквизита = МодульРаботаВМоделиСервиса.РазделительОсновныхДанных();
	ОбластьДанныхОсновныеДанныеСостав = МодульРаботаВМоделиСервиса.СоставОбщегоРеквизита(ИмяОбщегоРеквизита);
	ЭлементСоставаОбщегоРеквизита = ОбластьДанныхОсновныеДанныеСостав.Найти(ПустаяСсылка().Метаданные());
	...
	Если ЭлементСоставаОбщегоРеквизита <> Неопределено Тогда
		...
		Если ЭлементСоставаОбщегоРеквизита.Использование = Свойство_ОбъектИспользовать Тогда
			Возврат Ложь;
		ИначеЕсли ЭлементСоставаОбщегоРеквизита.Использование = Свойство_ОбъектАвто Тогда
			ОбластьДанныхОсновныеДанныеМетаданные = Метаданные.ОбщиеРеквизиты[ИмяОбщегоРеквизита];
			Если ОбластьДанныхОсновныеДанныеМетаданные.АвтоИспользование = Свойство_АвтоИспользовать Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ЗагрузитьДанныеДочернихКлассификаторовИзДополнительныхИсточников(ДанныеКлассификатора, ДанныеДочернихКлассификаторов)
	...
	РезультатЗагрузкиДочернихКлассификаторов = Новый Структура("Создано, Обновлено, Ошибки, Объекты");
	РезультатЗагрузкиДочернихКлассификаторов.Создано   = 0;
	РезультатЗагрузкиДочернихКлассификаторов.Обновлено = 0;
	РезультатЗагрузкиДочернихКлассификаторов.Ошибки    = Новый Массив;
	РезультатЗагрузкиДочернихКлассификаторов.Объекты   = Новый Соответствие;
...
Функция ЗагрузитьКТРУ(ДанныеКлассификатора, ДанныеЗагруженныхДочернихКлассификаторов)
	...
	Результат = Новый Структура("Создано, Обновлено, Отменено, Ошибки, Объекты");
	Результат.Создано   = 0;
	Результат.Обновлено = 0;
	Результат.Отменено  = 0;
	Результат.Ошибки    = Новый Массив;
	Результат.Объекты   = Новый Соответствие;
