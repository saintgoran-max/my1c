...
Функция ПолучитьПодпись(УчетнаяЗапись, ПодписьПисем = Неопределено) Экспорт
	...
	|ИЗ
	|	Справочник.УдалитьПодписиПисем КАК ПодписиПисем
	|ГДЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УдалитьПодписиПисем КАК ПодписиПисем
	|		ПО ОсновныеПодписиПисем.УдалитьПодпись = ПодписиПисем.Ссылка
	|ГДЕ
