...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		Если Параметры.Свойство("ПолитикаУчетаСерий") И ЗначениеЗаполнено(Параметры.ПолитикаУчетаСерий) Тогда
			Если Параметры.ПолитикаУчетаСерий.СоздаватьШК Тогда
				СоздаватьШК = Истина;
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДекорацияИнформацияПоШтрихкодам", "Видимость", Истина);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("СоздаватьШК", СоздаватьШК);
