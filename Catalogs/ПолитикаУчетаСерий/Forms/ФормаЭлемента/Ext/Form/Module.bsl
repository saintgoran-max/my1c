...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.ИспользоватьНеуникальныеШтрихКоды.Получить() Тогда
		Ссылка = ПолучитьНавигационнуюСсылку(
			...
			НСтр("ru = 'Неуникальные штрихкоды'"),
			Новый Структура("СтрокаПоиска", НСтр("ru = 'неуникальные штрихкоды'")));
			
		ТекстПодсказки = СтроковыеФункции.ФорматированнаяСтрока(
			НСтр("ru = 'Для корректной работы рекомендуется включить неуникальные штрихкоды в <a href=""%1"">настройках программы</a>.'"), Ссылка);
			
		Элементы.СоздаватьШК.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(
			Элементы.СоздаватьШК.РасширеннаяПодсказка.Заголовок,
			Символы.ПС,
			Символы.ПС,
			ТекстПодсказки);
	КонецЕсли;
