...
Функция ПроверитьНаличиеДопЛистовКнигиПродаж(СтруктураПараметров) Экспорт
	...
	КонецДня(ДатаПроверкиПараметровУчета), Новый Структура("Организация", СтруктураПараметров.Организация));
	ОрганизацияОсвобожденаОтУплатыНДС = СистемаНалогообложения.ПрименяетсяОсвобождениеОтУплатыНДС;
	
	СтруктураПараметров.Вставить("ПрименяетсяОсвобождениеОтУплатыНДС", 
		ОрганизацияОсвобожденаОтУплатыНДС);
...
Функция ЗаписиКнигиПродаж(СтруктураПараметров)
	...
	ПрослеживаемостьПереопределяемый.РегистрационныеСведенияНаДаты(Запрос.МенеджерВременныхТаблиц);
...
Функция ТекстЗапросаПодготовленныеЗаписиКнигиПродаж(СтруктураПараметров)
	...
	|							ИЛИ ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.СчетФактура).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка)
	...
	|					И ВЫРАЗИТЬ(ЗаписиКнигиПродаж.СчетФактура КАК Документ.ОперацияПоПлатежнымКартам).ВидОперации = &ОПК_РозничнаяВыручка
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
