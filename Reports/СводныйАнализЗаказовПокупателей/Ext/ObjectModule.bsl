...
Процедура ЗаполнитьСпискиВыбораПриОпределенииНастроек(Форма)
	...
	ЗаполнитьСписокВыбораСостоянийОтгрузки(Форма);
...
Процедура ЗаполнитьСписокВыбораСостоянийОтгрузки(Форма)
	...
	ЕстьПолеВыбораСостояния = Ложь;
	ПараметрКомпоновки = Новый ПараметрКомпоновкиДанных("СостояниеОтгрузки");
	Для Каждого ЭлементНастройки Из Форма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
	...
	ЭлементФормы = Форма.Элементы[ИмяЭлемента];
	Если ТипЗнч(ЭлементФормы) <> Тип("ПолеФормы") Или ЭлементФормы.Заголовок <> "Состояние отгрузки" Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ЭлементСписка Из ЭлементФормы.СписокВыбора Цикл
		...
		Если ЭлементСписка.Значение = "Не отгруженные" Тогда
			ЭлементСписка.Представление = НСтр("ru = 'Без отгрузки'");
		ИначеЕсли ЭлементСписка.Значение = "Отгруженные частично" Тогда
			ЭлементСписка.Представление = СтрШаблон("%1%2", Отступ, НСтр("ru = 'Отгружен частично'"));
		ИначеЕсли ЭлементСписка.Значение = "Отгруженные полностью" Тогда
			ЭлементСписка.Представление = СтрШаблон("%1%2", Отступ, НСтр("ru = 'Отгружен полностью'"));
		ИначеЕсли ЭлементСписка.Значение = "Без полной отгрузки" Тогда
			ЭлементСписка.Представление = НСтр("ru = 'Без отгрузки или отгружен частично'");
		КонецЕсли;
