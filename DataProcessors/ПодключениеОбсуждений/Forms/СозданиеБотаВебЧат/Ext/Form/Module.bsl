...
Асинх Процедура СохранитьФайлВебСтраницыПродолжение(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗаписи = Истина Тогда
		...
		ТекстПояснения = НСтр("ru='Файл сохранен в %1'");
		Пояснение = СтрШаблон(ТекстПояснения, ВыбранныеФайлы[0]);
		...
		ПоказатьОповещениеПользователя(ТекстОповещения, ОписаниеОповещения, Пояснение, БиблиотекаКартинок.Информация32);
