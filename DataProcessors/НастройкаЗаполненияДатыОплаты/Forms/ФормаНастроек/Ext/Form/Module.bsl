...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНастройкаПоДоговору Тогда
		РаботаСФормойДокумента.НастроитьВидимостьГруппыИнформации(ЭтотОбъект, "ГруппаИнформацияПоНастройкам",
							"ПоказыватьИнформациюПоНастройкамДатыОплаты");
	КонецЕсли;
...
&НаКлиенте
Процедура ВариантЗаполненияДатыОплатыПриИзменении(Элемент)
	
	Если ВариантЗаполненияДатыОплаты =
		...
		Если НЕ ЗначениеЗаполнено(ВидДнейРасчетаДатыОплаты) Тогда
			ВидДнейРасчетаДатыОплаты = ПредопределенноеЗначение("Перечисление.ВидыДнейРасчетаДатыОтгрузки.Календарные");
		КонецЕсли;
		Элементы.ГруппаПроизводственныйКалендарь.Видимость =
			(ВидДнейРасчетаДатыОплаты = ПредопределенноеЗначение("Перечисление.ВидыДнейРасчетаДатыОтгрузки.Рабочие"));
	Иначе
...
Процедура ВидДнейПриИзменении(Элемент)
	...
	Если ВидДнейРасчетаДатыОплаты = ПредопределенноеЗначение("Перечисление.ВидыДнейРасчетаДатыОтгрузки.Рабочие") Тогда 
...
Процедура ДекорацияИнформацияПоНастройкамОбработкаНавигационнойСсылки(Элемент,
	...
	ПараметрыФормы = Новый Структура("СтрокаПоиска", НСтр("ru='+настройки +заполнения +даты +оплаты'"));
...
Процедура ОК(Команда)
	...
	Если ВариантЗаполненияДатыОплаты = 
			...
			И ВидДнейРасчетаДатыОплаты = ПредопределенноеЗначение("Перечисление.ВидыДнейРасчетаДатыОтгрузки.Рабочие")
			И ПроизводственныйКалендарьРасчетаДатыОплаты.Пустая() Тогда
...
Процедура ЗаполнитьРеквизитыФормы(ПоУмолчанию = Ложь)
	...
	СписокСвойств = Обработки.НастройкаЗаполненияДатыОплаты.СписокНастроекДатыОплаты();
	...
	Если ВариантЗаполненияДатыОплаты.Пустая() Тогда
		ВариантЗаполненияДатыОплаты = Перечисления.ВариантыЗаполненияДатыОтгрузки.НеЗаполнять;
		ВидДнейРасчетаДатыОплаты = Перечисления.ВидыДнейРасчетаДатыОтгрузки.Календарные;
	КонецЕсли;
...
Процедура УстановитьТекстДекорацииДней()
	...
	ПолнаяСтрокаКалендарные = ПолучитьСклоненияСтрокиПоЧислу(КалендарныйДень, КоличествоДнейРасчетаДатыОплаты,,
	...
	ПолнаяСтрокаРабочие = ПолучитьСклоненияСтрокиПоЧислу(РабочийДень, КоличествоДнейРасчетаДатыОплаты,,
		"ЧС=Количественное", "ПД=Именительный");
	...
	СтрокаЗаголовка = НСтр("ru='%1 к дате документа'");
...
Функция ПараметрыЗакрытия()
	...
	Результат = Обработки.НастройкаЗаполненияДатыОплаты.СтруктураНастроекДатыОплаты();
...
Процедура СохранитьНастройкуПоказыватьИнформациюПоНовойСхеме()
	...
	РаботаСФормойДокумента.СохранитьВидимостьГруппыИнформации(ИмяФормы,
			"ПоказыватьИнформациюПоНастройкамДатыОплаты", Ложь);
