...
Процедура ВывестиГруппуВТабличныйДокументПолотно(КоллекцияСтрок, ПараметрыФормирования, ТабличныйДокумент)
	...
	ПараметрыСжатияИзображений = ИнициализироватьПараметрыСжатияИзображений();
		...
		Если ВыводитьКартинкиНоменклатуры Тогда
			...
			Если ЗначениеЗаполнено(Строка.КартинкаНоменклатуры) Тогда
				...
				Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
					...
					ВыводимоеИзображение = Новый Картинка(ДанныеКартинки);	
					Если ПараметрыФормирования.Свойство("СжиматьИзображения") 
						И ПараметрыФормирования.СжиматьИзображения Тогда 
						
						// выполним сжатие картинки
						ОбластьСтрока.Область("КартинкаНоменклатуры").Картинка = СжатьКартинку(ВыводимоеИзображение, 
							ПараметрыСжатияИзображений);
						
					Иначе 
						// выведем картинку без сжатия
						ОбластьСтрока.Область("КартинкаНоменклатуры").Картинка = ВыводимоеИзображение;
			...
			Если ЗначениеЗаполнено(Строка.КартинкаХарактеристики) Тогда
				...
				Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
					...
					Если ПараметрыФормирования.Свойство("СжиматьИзображения") 
						...
						// выполним сжатие картинки
						ОбластьСтрока.Область("КартинкаХарактеристики").Картинка = СжатьКартинку(ВыводимоеИзображение, 
							ПараметрыСжатияИзображений);
						
					Иначе 
						// выведем картинку без сжатия
						ОбластьСтрока.Область("КартинкаХарактеристики").Картинка = ВыводимоеИзображение;
...
Процедура ВывестиГруппуВТабличныйДокументДиафильм(КоллекцияСтрок, ПараметрыФормирования, ТабличныйДокумент)
	...
	ВидЦен 						= ?(ПараметрыФормирования.ТаблицаВидовЦен.Количество() = 0, Неопределено, 
		ПараметрыФормирования.ТаблицаВидовЦен[0].ВидЦен);
				...
				Если ЗначениеЗаполнено(ДанныеКартинки) Тогда
					...
					Если ПараметрыФормирования.Свойство("СжиматьИзображения") 
						...
						ОбластьСлайдДетали.Область("Картинка"
							+ СХарактеристикой).Картинка = СжатьКартинку(ВыводимоеИзображение, ПараметрыСжатияИзображений); 
						
					Иначе 
						// выведем картинку без сжатия
						ОбластьСлайдДетали.Область("Картинка" 
							+ СХарактеристикой).Картинка = ВыводимоеИзображение;
...
Функция СжатьКартинку(СжимаемаяКартинка, ПараметрыСжатия) 
	
	СжатаяКартинка 		= СжимаемаяКартинка; 		
	МаксимальнаяВысота 	= ПараметрыСжатия.МаксимальнаяВысота; 	
	
	ТекущаяВысота = СжимаемаяКартинка.Высота();
	Если ТекущаяВысота > МаксимальнаяВысота И МаксимальнаяВысота > 0 Тогда 
		
		ОбрабатываемаяКартинка = Новый ОбрабатываемаяКартинка(СжимаемаяКартинка);
		ОбрабатываемаяКартинка.УстановитьРазмер(Неопределено, МаксимальнаяВысота);
		
		СжатаяКартинка = ОбрабатываемаяКартинка.ПолучитьКартинку();
		Возврат СжатаяКартинка;
...
Функция ИнициализироватьПараметрыСжатияИзображений() 
	
	ПараметрыСжатия = Новый Структура;
	ПараметрыСжатия.Вставить("МаксимальнаяВысота", 80);	
	ПараметрыСжатия.Вставить("МаксимальнаяШирина", 80);
	
	Возврат ПараметрыСжатия;
