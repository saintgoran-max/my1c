...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	РегистрыСведений.СоставыГруппПользователей.ОбновитьИерархиюИСоставы();
...
Процедура ПКО_ПослеЗагрузки_ВводОстатковСобственныеТоварыНаСкладах(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого ТекСтрока Из Объект.Запасы Цикл
		...
		Если НЕ ЗначениеЗаполнено(ТекСтрока.СтруктурнаяЕдиница) Тогда
			ТекСтрока.СтруктурнаяЕдиница = Справочники.СтруктурныеЕдиницы.ОсновнойСклад;
		КонецЕсли;
...
Процедура ПослеЗагрузкиПрисоединенногоФайла(Объект) Экспорт
	...
	Объект.ИндексКартинки				= РаботаСФайламиСлужебныйКлиентСервер.ИндексПиктограммыФайла(Объект.Расширение);
