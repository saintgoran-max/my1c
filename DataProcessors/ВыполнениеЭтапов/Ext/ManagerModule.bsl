...
Функция ВыполнитьЭтапСборкаЗапасов(ДокументОбъект, ЗначенияПолей, Ошибки)
	...
	СтруктураПроверки = ПроверитьСозданиеСтроки(ЗначенияПолей,ДокументОбъект);
	
	Если СтруктураПроверки.НовыйЭтап Тогда
	...
	Иначе
		...
		НоваяСтрокаПродукция = СтруктураПроверки.СтрокиЗаполнения;
	КонецЕсли;
...
Функция ПроверитьСозданиеСтроки(ЗначенияПолей,ДокументОбъект)
	СтруктураПоиска = Новый Структура;
	...
	//Этапы
	НовыйЭтап = Истина;
	Если НЕ СтрокиПродукция.Количество()=0 тогда
	СтруктураПоискаЭтапа = Новый Структура;
	СтруктураПоискаЭтапа.Вставить("КлючСвязи",СтрокиПродукция[0].КлючСвязи);
	СтруктураПоискаЭтапа.Вставить("Этап",ЗначенияПолей.Этап);
	
	НовыйЭтап = НЕ ДокументОбъект.ВыполненныеЭтапы.НайтиСтроки(СтруктураПоискаЭтапа).Количество() = 0;
	СтруктураВозврата.Вставить("СтрокиЗаполнения",СтрокиПродукция[0]);
	КонецЕсли;
	СтруктураВозврата.Вставить("НовыйЭтап",НовыйЭтап);
