...
Функция ИдентификаторПечатнойФормы(Подробно = Ложь, ИспользоватьФаксимиле = Ложь) Экспорт
	...
	Возврат ?(Подробно, "АктОбОказанииУслугПодробно", "АктОбОказанииУслуг");
...
//  Строка
//
Функция СинонимМакета(Подробно = Ложь) Экспорт
...
Функция СформироватьПФ(ОписаниеПечатнойФормы, ДанныеОбъектовПечати, ОбъектыПечати, Подробно) Экспорт
	...
	ТекстСообщенияМакетИзменен = ПечатьДокументовУНФ.ТекстСообщенияМакетИзменен(НСтр("ru = 'Акт об оказании услуг'"));
	...
	Для Каждого ДанныеОбъекта Из ДанныеОбъектовПечати Цикл
		...
		Если ОбластьСтрока <> Неопределено Тогда
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОбъекта, "ВидЗакрывающегоДокумента") Тогда
				ЭтоАкт = ДанныеОбъекта.ВидЗакрывающегоДокумента = Перечисления.ДоступныеПечатныеФормыВыставлениеСчетов.Акт;
			Иначе
				ЭтоАкт = Истина;
			КонецЕсли;
			...
			Для Каждого СтрокаРаботыУслуги Из ДанныеОбъекта.ТаблицаРаботыУслуги Цикл
				...
				Если СтрокаРаботыУслуги.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Услуга
					И СтрокаРаботыУслуги.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Работа
					И НЕ (ИнтерфейсВыставлениеСчетов И ЭтоАкт) Тогда
