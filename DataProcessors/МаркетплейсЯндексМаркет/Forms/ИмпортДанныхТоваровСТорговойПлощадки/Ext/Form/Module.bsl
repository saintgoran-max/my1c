...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = НСтр("ru = 'Импорт описаний товаров с торговой площадки Яндекс Маркет'") + ", " + СокрЛП( УчетнаяЗаписьМаркетплейса );
...
Процедура ИмпортируемыеТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	И ЗначениеЗаполнено( ТекущиеДанные.ИдентификаторОбъектаМаркетплейса )  Тогда
	    ...
	    URL = СтрШаблон( ШаблонURL, ЭтаФорма.ИдентификаторКампанииЯндексМаркет, ТекущиеДанные.ИдентификаторОбъектаМаркетплейса );
	...
	И ЗначениеЗаполнено( ТекущиеДанные.ИдентификаторПубликации )  Тогда
	    ...
	    ШаблонURL = "https://partner.market.yandex.ru/supplier/%1/assortment/offer-card?article=%2";
	    URL = СтрШаблон( ШаблонURL, ЭтаФорма.ИдентификаторКампанииЯндексМаркет, ТекущиеДанные.ИдентификаторПубликации );
	    ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку( URL );
...
Процедура ФормаРежимОтображенияЭлементыОбновить( ПараметрФормаРежимОтображения )
	...
	Если ЭтаФорма.ФормаРежимОтображенияТекущий = ИнтеграцияСЯндексМаркетКлиентСервер.ФормаРежимОтображенияОткрытие() Тогда

		Элементы.ГруппаНазадВперед.Видимость = Ложь;

		Если ЭтаФорма.ВариантЗагрузкиДанных = ИнтеграцияСЯндексМаркетКлиентСервер.ВариантЗагрузкиОбновитьСопоставленныеКарточкиТоваров() Тогда 
		...
		ИначеЕсли ЭтаФорма.ВариантЗагрузкиДанных = ИнтеграцияСЯндексМаркетКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров() Тогда 
	...
	ИначеЕсли ЭтаФорма.ФормаРежимОтображенияТекущий = ИнтеграцияСЯндексМаркетКлиентСервер.ФормаРежимОтображенияЗагрузитьВсеОписания() Тогда
	...
	ИначеЕсли ЭтаФорма.ФормаРежимОтображенияТекущий = ИнтеграцияСЯндексМаркетКлиентСервер.ФормаРежимОтображенияЗагрузитьСДетальнымиНастройками() Тогда
...
Функция ПолучитьДанныеСТорговойПлощадкиНаСервере( КэшКатегорий, Знач СледующаяПорцияДанных = Ложь)
	...
	Если ЭтаФорма.ВариантЗагрузкиДанных = ИнтеграцияСЯндексМаркетКлиентСервер.ВариантЗагрузкиЗагрузитьНесопоставленныеКарточкиТоваров() Тогда
...
Процедура ОбновитьСтрокуДереваТоваров(ИсходныйКлючЗаписи, КлючЗаписи)
	...
	НастройкиУчетнойЗаписи = Справочники.УчетныеЗаписиМаркетплейсов.НастройкиУчетнойЗаписи(УчетнаяЗаписьМаркетплейса);
	ВидыЦен                = ИнтеграцияСЯндексМаркетСервер.ВидыЦенУчетнойЗаписи(УчетнаяЗаписьМаркетплейса, Истина, Истина);
