...
Процедура ПроверитьЗаписатьДанныеПоДоставке(ДокументОснование, ТипГрузоперевозки, ИдентификаторЗаказа = "", ТрекНомер = "") Экспорт
	...
	Иначе
		Если ТипГрузоперевозки = Перечисления.СервисыДоставки.Сервис1СДоставка Тогда
			НоваяЗаписьРегистра.ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СДоставка();
		ИначеЕсли ТипГрузоперевозки = Перечисления.СервисыДоставки.Сервис1СКурьер Тогда
			НоваяЗаписьРегистра.ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьер();
		ИначеЕсли ТипГрузоперевозки = Перечисления.СервисыДоставки.Сервис1СКурьерика Тогда
			НоваяЗаписьРегистра.ТипГрузоперевозки = СервисДоставкиКлиентСервер.ТипГрузоперевозкиСервис1СКурьерика();
		КонецЕсли;
	КонецЕсли;
