...
Процедура РассчитатьНалогиИВзносы()
	...
	Запрос.УстановитьПараметр("ТарифМСП",
		РегламентированнаяОтчетностьПереопределяемый.СсылкаНаОбъектПоИдентификатору(
		"Справочник.ВидыТарифовСтраховыхВзносовУНФ", "ДляСубъектовМалогоПредпринимательства"));
	...
	|			КОГДА НеОграниченнаяОблагаемаяБаза.ВидТарифаСтраховыхВзносов В (&ОбщийНалоговыйРежим, &УпрощенныйНалоговыйРежим, &ТарифМСП)
	...
	|			КОГДА НеОграниченнаяОблагаемаяБаза.ВидТарифаСтраховыхВзносов В (&ОбщийНалоговыйРежим, &УпрощенныйНалоговыйРежим, &ТарифМСП, &ЕНВД)
	|				ТОГДА ЕСТЬNULL(НеОграниченнаяОблагаемаяБаза.СуммаПревысившаяПределПФР, 0) * ЕСТЬNULL(ОсновнойТариф.СтавкаСПревышенияПредела / 100, 0.1)
	|			ИНАЧЕ 0
