...
Процедура ЗаполнитьПоНакладнойЕГАИС(ТТНВходящаяЕГАИС) Экспорт
	...
	Если ЭтотОбъект.ЕстьРасхождения Тогда
		ЭтотОбъект.ПринятьПоКолонке = ПредопределенноеЗначение("Перечисление.ПринятьПоКолонке.КоличествоПоФакту");
	КонецЕсли;
...
Процедура ЗаполнитьНомерСтрокиДокументаОснования(Отказ)
	...
	ИспользоватьЦену = Не ДокументОснование.СкидкиНаценки.Количество() 
						Или ДокументОснование.СкидкиНаценки.Количество() И ПолучитьФункциональнуюОпцию("ИспользоватьРучныеСкидкиНаценкиЗакупки");
	
	ДополнениеЦена = ?(ИспользоватьЦену, ", Цена", "");
		
	ТаблицаНоменклатурыОснования = ДокументОснование.Запасы.Выгрузить(,"Номенклатура, Характеристика, Партия, Количество, ЕдиницаИзмерения" + ДополнениеЦена);
	...
	ТаблицаНоменклатурыВозврата = Запасы.Выгрузить(,"Номенклатура, Характеристика, Партия, Количество, ЕдиницаИзмерения" + ДополнениеЦена);
	...
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Партия" + ДополнениеЦена);
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаНоменклатурыВозврата Цикл
		...
		Если НайденныеСтроки.Количество() И НайденныеСтроки[0].Количество < СтрокаТабличнойЧасти.Количество Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Количество номенклатуры %1 (%2) превышает количество в документе основания (%3).'"), 
	...
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Партия, НоменклатураНабора, ХарактеристикаНабора, ЗаказПокупателя" + ДополнениеЦена);
		...
		Иначе
			...
			Иначе
				...
				Если ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") 
					...
					КоличествоСтроки = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.ЕдиницаИзмерения.Коэффициент;
				Иначе
...
Процедура ПроверитьКоэффициентыЕдиницИзмерения(ТаблицаНоменклатуры);
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаНоменклатуры Цикл
		...
		Если ТипЗнч(СтрокаТабличнойЧасти.ЕдиницаИзмерения) = Тип("СправочникСсылка.ЕдиницыИзмерения") 
			И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения) Тогда
			СтрокаТабличнойЧасти.Количество = СтрокаТабличнойЧасти.Количество * СтрокаТабличнойЧасти.ЕдиницаИзмерения.Коэффициент;
		КонецЕсли;
