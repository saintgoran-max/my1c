...
Процедура СформироватьТаблицаЗапасыПродажа(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;	
...
Процедура СформироватьТаблицаДоходыИРасходы(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	|ИЗ
	|	(" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийДоходыИРасходы(3, ВыполнятьДвиженияПоСертификатам)+") КАК ДанныеСертификатов
	|";	
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;	
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийРасчетыСПокупателями(ВыполнятьДвиженияПоСертификатам);
...
Процедура СформироватьТаблицаУправленческий(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;	
	ТекстЗапросаСертификаты = СтрЗаменить(ТекстЗапросаСертификаты, "ВложенныйЗапросСертификаты", "(" + РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийУправлченческий(12, ВыполнятьДвиженияПоСертификатам) + ")");
	...
	ТаблицаДокумента.Свернуть("Период, Организация, СценарийПланирования, СчетДт, СчетКт, ВалютаДт, ВалютаКт, Содержание",
...
Процедура СформироватьТаблицаПартииТоваровУСН(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю Тогда
		...
		УчетПартийУСН.ЗаполнитьДвиженияСписаниеПартий(ДокументСсылкаРасходнаяНакладная, ПартииСписать, ТаблицаЗапасы, ТаблицаПартииТоваров);
		...
		Если ТаблицаПартииТоваров.Найти(Истина, "ОтражатьВКУДиР") <> Неопределено Тогда
			...
			Если ТаблицаЗапасы.Количество() > 0 Тогда
				ТекстПокупатель = ?(ПустаяСтрока(ТаблицаЗапасы[0].ЮридическоеНаименованиеКонтрагента),
						ТаблицаЗапасы[0].НаименованиеКонтрагента,
						ТаблицаЗапасы[0].ЮридическоеНаименованиеКонтрагента);
			КонецЕсли;
			СтрокаОписание.Содержание = УчетПартийУСНКлиентСервер.СодержаниеПродажаТоваров(ТекстПокупатель);
		КонецЕсли;
		
	ИначеЕсли СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВозвратПоставщику Тогда
			...
			Пока ВыборкаПартии.Следующий() Цикл
				...
				Для Каждого СтрокаЗапас Из СтрокиЗапаса Цикл
					...
					Если СтрокаЗапас.Количество < РесурсыСписать.Количество Тогда
						Коэффициент = ?(ВыборкаПартии.Количество = 0, 1, СтрокаЗапас.Количество / РесурсыСписать.Количество);
...
Процедура СформироватьТаблицаСерииНоменклатуры(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаДвиженияСерииНоменклатуры", ТаблицаДвиженияСерий);
	...
	ТаблицаСерииНоменклатуры = РезультатЗапроса.Скопировать(ОстаткиСерийНоменклатурыСтроки);
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства) Экспорт
	...
	Если СтруктураДополнительныеСвойства.ДляПроведения.ОстаткиЗапасовНеИспользуются Тогда
		ПроведениеДокументовУНФ.ДополнитьТаблицуОбратнымиДвижениями(СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасы);
		ПроведениеДокументовУНФ.ДополнитьТаблицуОбратнымиДвижениями(СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЗапасыНаСкладах);
	КонецЕсли;
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Цена КАК ЧИСЛО(15, 2))) * РасходнаяНакладнаяЗапасы.Ссылка.Курс / РасходнаяНакладнаяЗапасы.Ссылка.Кратность КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВЫБОР
	...
	|							ИНАЧЕ ВЫРАЗИТЬ((СуммыДокументовРегламентированныйУчет.Всего - СуммыДокументовРегламентированныйУчет.НДС) / (ВЫРАЗИТЬ(РасходнаяНакладнаяЗапасы.Количество КАК ЧИСЛО(15, 2))) КАК ЧИСЛО(15, 2))
	|						КОНЕЦ
	|				КОНЕЦ
	|		ИНАЧЕ РасходнаяНакладнаяЗапасы.Цена
	|	КОНЕЦ КАК Цена,
	...
	|						ТОГДА ВЫРАЗИТЬ((ИтоговаяТаблицаЗапасы.Сумма - ИтоговаяТаблицаЗапасы.СуммаНДС) / ИтоговаяТаблицаЗапасы.КоличествоДляПрослеживаемости КАК ЧИСЛО(15, 2))
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаТаблицы Из ВозможныеВарианты Цикл
		...
		Если ПечатнаяФорма <> Неопределено Тогда
			...
			Обработки.ПечатьТОРГ12.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати, СтрокаТаблицы.ВключаяУслуги, ВыводитьФаксимиле);
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Обработки.ПечатьСчетНаОплату.СформироватьПФ(ПечатнаяФорма, ДанныеОбъектовПечати, ОбъектыПечати, Ложь, ВыводитьФаксимиле);
...
Процедура СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаРасходнаяНакладная, СтруктураДополнительныеСвойства)
	...
	ВыполнятьДвиженияПоСертификатам = СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю
		Или СтруктураДополнительныеСвойства.ДляПроведения.ВидОперации = Перечисления.ВидыОперацийРасходнаяНакладная.ВыкупТоваровКомиссионером;	
	Запрос.Текст = РаботаСПодарочнымиСертификатами.ТекстЗапросаДвиженийОплатыПодарочнымиСертификатами(ВыполнятьДвиженияПоСертификатам);
	Результат = Запрос.Выполнить().Выгрузить();
