...
Процедура ПодготовитьДанныеДляЗаполнения(ПараметрыДляЗаполнения, АдресХранилища) Экспорт
	...
	СтруктураПараметров.Вставить("СписокОрганизаций",
		УчетНДСУНФВызовСервераПовтИсп.ПолучитьСписокОбособленныхПодразделений(СтруктураПараметров.Организация));
...
Функция ПечатьДополнительногоЛистаКнигиПокупок(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТабличнаяЧастьДопЛиста.НомерСтроки КАК НомерСтроки,
	|	ТабличнаяЧастьДопЛиста.ДатаНомер КАК ДатаНомер,
	|	ТабличнаяЧастьДопЛиста.НомерДатаИсправления КАК НомерДатаИсправления,
	|	ТабличнаяЧастьДопЛиста.НомерДатаКорректировки КАК НомерДатаКорректировки,
	|	ТабличнаяЧастьДопЛиста.НомерДатаИсправленияКорректировки КАК НомерДатаИсправленияКорректировки,
	|	ТабличнаяЧастьДопЛиста.ДатаОплаты КАК ДатаОплаты,
	|	ТабличнаяЧастьДопЛиста.ДатаОприходования КАК ДатаОприходования,
	|	ТабличнаяЧастьДопЛиста.Продавец КАК Продавец,
	|	ТабличнаяЧастьДопЛиста.ПродавецИНН КАК ПродавецИНН,
	|	ТабличнаяЧастьДопЛиста.ПродавецКПП КАК ПродавецКПП,
	|	ТабличнаяЧастьДопЛиста.НомерГТД КАК НомерГТД,
	|	ТабличнаяЧастьДопЛиста.ВсегоПокупок КАК ВсегоПокупок,
	|	ТабличнаяЧастьДопЛиста.СуммаБезНДС18 КАК СуммаБезНДС18,
	|	ТабличнаяЧастьДопЛиста.НДС18 КАК НДС18,
	|	ТабличнаяЧастьДопЛиста.СуммаБезНДС10 КАК СуммаБезНДС10,
	|	ТабличнаяЧастьДопЛиста.НДС10 КАК НДС10,
	|	ТабличнаяЧастьДопЛиста.НДС0 КАК НДС0,
	|	ТабличнаяЧастьДопЛиста.СуммаСовсемБезНДС КАК СуммаСовсемБезНДС,
	|	ТабличнаяЧастьДопЛиста.Ном КАК Ном,
	|	ТабличнаяЧастьДопЛиста.СчетФактура КАК СчетФактура,
	...
	|	ДопЛистКнигиПокупокДляПередачиВЭлектронномВиде.Организация.ИндивидуальныйПредприниматель КАК ОрганизацияИндивидуальныйПредприниматель
	|ИЗ
	|	Документ.ДопЛистКнигиПокупокДляПередачиВЭлектронномВиде КАК ДопЛистКнигиПокупокДляПередачиВЭлектронномВиде
	...
	Пока ВыборкаПоОбъектам.Следующий() Цикл
		...
		СтруктураПараметров.Вставить("СписокОрганизаций", УчетНДСУНФВызовСервераПовтИсп.ПолучитьСписокОбособленныхПодразделений(ВыборкаПоОбъектам.Организация));
