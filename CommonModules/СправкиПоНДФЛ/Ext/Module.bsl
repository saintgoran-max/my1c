...
Функция СформироватьПечатнуюФорму2НДФЛ(ОбъектыПечати, МассивОбъектов, ДанныеНА, ДанныеСотрудниковДокументов, ДанныеОДоходахДокументов, ДанныеОВычетахДокументов, ДанныеОбУведомленияхДокументов) Экспорт
	...
	ОбластьМакетаШапкаВторойСтраницы_2022 = Справка2022.ПолучитьОбласть("ШапкаВторойСтраницы");
	...
	Справка2024 = ПолучитьОбщийМакет("Форма2НДФЛс2024");
	ОбластьМакетаШапкаПервойСтраницы_2024 = Справка2024.ПолучитьОбласть("ШапкаПервойСтраницы"); 
	ОбластьМакетаШапкаВторойСтраницы_2024 = Справка2024.ПолучитьОбласть("ШапкаВторойСтраницы");
	ОбластьМакетаЗаголовокРаздела3_2024 = Справка2024.ПолучитьОбласть("ЗаголовокРаздела3"); 
	ОбластьМакетаЗаголовокРаздела4_2024 = Справка2024.ПолучитьОбласть("ЗаголовокРаздела4");
	ОбластьМакетаРаздел5_2024 = Справка2024.ПолучитьОбласть("Раздел5");
	ОбластьМакетаРаздел6_2024 = Справка2024.ПолучитьОбласть("Раздел6");
	ОбластьМакетаНалоговыйАгент_2024 = Справка2024.ПолучитьОбласть("НалоговыйАгент");
	ОбластьМакетаПримечание_2024 = Справка2024.ПолучитьОбласть("Примечание");
	
	ОбластьМакетаСтрокаВычетов_2024 = Справка2024.ПолучитьОбласть("СтрокаВычетов"); 
	ОбластьМакетаСтрокаДоходов_2024 = Справка2024.ПолучитьОбласть("СтрокаДоходов"); 
	ОбластиМакетаДляДоходов_2024 = Новый Массив;
	ОбластиМакетаДляДоходов_2024.Добавить(Справка2024.ПолучитьОбласть("СтрокаДоходов|ЛеваяЧасть"));
	ОбластиМакетаДляДоходов_2024.Добавить(Справка2024.ПолучитьОбласть("СтрокаДоходов|ПраваяЧасть"));
	ОбластиМакетаДляВычетов_2024 = Новый Массив;
	Для Сч = 1 По 4 Цикл
		ОбластиМакетаДляВычетов_2024.Добавить(Справка2024.ПолучитьОбласть("СтрокаВычетов|Вычеты"+Сч));
	КонецЦикла;
	
	Справка2024_2 = ПолучитьОбщийМакет("Форма2НДФЛсо2кв2024");
	ОбластьМакетаШапкаПервойСтраницы_2024_2 = Справка2024_2.ПолучитьОбласть("ШапкаПервойСтраницы"); 
	ОбластьМакетаШапкаВторойСтраницы_2024_2 = Справка2024_2.ПолучитьОбласть("ШапкаВторойСтраницы");
	ОбластьМакетаЗаголовокРаздела3_2024_2 = Справка2024_2.ПолучитьОбласть("ЗаголовокРаздела3"); 
	ОбластьМакетаЗаголовокРаздела4_2024_2 = Справка2024_2.ПолучитьОбласть("ЗаголовокРаздела4");
	ОбластьМакетаРаздел5_2024_2 = Справка2024_2.ПолучитьОбласть("Раздел5");
	ОбластьМакетаРаздел6_2024_2 = Справка2024_2.ПолучитьОбласть("Раздел6");
	ОбластьМакетаНалоговыйАгент_2024_2 = Справка2024_2.ПолучитьОбласть("НалоговыйАгент");
	ОбластьМакетаПримечание_2024_2 = Справка2024_2.ПолучитьОбласть("Примечание");
	
	ОбластьМакетаСтрокаВычетов_2024_2 = Справка2024_2.ПолучитьОбласть("СтрокаВычетов"); 
	ОбластьМакетаСтрокаДоходов_2024_2 = Справка2024_2.ПолучитьОбласть("СтрокаДоходов"); 
	ОбластиМакетаДляДоходов_2024_2 = Новый Массив;
	ОбластиМакетаДляДоходов_2024_2.Добавить(Справка2024_2.ПолучитьОбласть("СтрокаДоходов|ЛеваяЧасть"));
	ОбластиМакетаДляДоходов_2024_2.Добавить(Справка2024_2.ПолучитьОбласть("СтрокаДоходов|ПраваяЧасть"));
	ОбластиМакетаДляВычетов_2024_2 = Новый Массив;
	Для Сч = 1 По 4 Цикл
		ОбластиМакетаДляВычетов_2024_2.Добавить(Справка2024_2.ПолучитьОбласть("СтрокаВычетов|Вычеты"+Сч));
	КонецЦикла;
	
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		
		ДанныеНалоговогоАгента = ДанныеНА.Получить(Ссылка);
		ВыводитьФормуСРазделомНеудержанногоНалога = Ложь;
		Если ВыводитьФорму2НДФЛ2024Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
			ВыводитьФормуСРазделомНеудержанногоНалога = Истина;
			Если ДанныеНалоговогоАгента.Дата < '20240409' Тогда
				ОбластьМакетаШапкаПервойСтраницы = ОбластьМакетаШапкаПервойСтраницы_2024; 
				ОбластьМакетаШапкаВторойСтраницы = ОбластьМакетаШапкаВторойСтраницы_2024;
				ОбластьМакетаЗаголовокРаздела3 = ОбластьМакетаЗаголовокРаздела3_2024; 
				ОбластьМакетаЗаголовокРаздела4 = ОбластьМакетаЗаголовокРаздела4_2024;
				ОбластьМакетаУведомлениеНаСоцВычет = Неопределено;
				ОбластьМакетаУведомлениеНаИмущВычет = Неопределено;
				ОбластьМакетаРаздел5 = ОбластьМакетаРаздел5_2024;
				ОбластьМакетаРаздел6 = ОбластьМакетаРаздел6_2024;
				ОбластьМакетаНалоговыйАгент = ОбластьМакетаНалоговыйАгент_2024;
				ОбластьМакетаПримечание = ОбластьМакетаПримечание_2024;
				
				ОбластьМакетаСтрокаВычетов = ОбластьМакетаСтрокаВычетов_2024; 
				ОбластьМакетаСтрокаДоходов = ОбластьМакетаСтрокаДоходов_2024; 
				ОбластиМакетаДляДоходов = ОбластиМакетаДляДоходов_2024;
				ОбластиМакетаДляВычетов = ОбластиМакетаДляВычетов_2024;
			Иначе
				ОбластьМакетаШапкаПервойСтраницы = ОбластьМакетаШапкаПервойСтраницы_2024_2;
				ОбластьМакетаШапкаВторойСтраницы = ОбластьМакетаШапкаВторойСтраницы_2024_2;
				ОбластьМакетаЗаголовокРаздела3 = ОбластьМакетаЗаголовокРаздела3_2024_2; 
				ОбластьМакетаЗаголовокРаздела4 = ОбластьМакетаЗаголовокРаздела4_2024_2;
				ОбластьМакетаУведомлениеНаСоцВычет = Неопределено;
				ОбластьМакетаУведомлениеНаИмущВычет = Неопределено;
				ОбластьМакетаРаздел5 = ОбластьМакетаРаздел5_2024_2;
				ОбластьМакетаРаздел6 = ОбластьМакетаРаздел6_2024_2;
				ОбластьМакетаНалоговыйАгент = ОбластьМакетаНалоговыйАгент_2024_2;
				ОбластьМакетаПримечание = ОбластьМакетаПримечание_2024_2;
				
				ОбластьМакетаСтрокаВычетов = ОбластьМакетаСтрокаВычетов_2024_2; 
				ОбластьМакетаСтрокаДоходов = ОбластьМакетаСтрокаДоходов_2024_2; 
				ОбластиМакетаДляДоходов = ОбластиМакетаДляДоходов_2024_2;
				ОбластиМакетаДляВычетов = ОбластиМакетаДляВычетов_2024_2;
			КонецЕсли;
		ИначеЕсли ВыводитьФорму2НДФЛ2022Года(ДанныеНалоговогоАгента.НалоговыйПериод, ДанныеНалоговогоАгента.Дата) Тогда
			ВыводитьФормуСРазделомНеудержанногоНалога = Истина;
		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Если ДанныеНалоговогоАгента.НалоговыйПериод >= 2015 Тогда
				ОбластьМакетаРаздел5.Параметры.ЗачтеноАвансовыхПлатежей = 0;
			КонецЕсли;
			
			Если ВыводитьФормуСРазделомНеудержанногоНалога Тогда
				ОбластьМакетаРаздел5.Параметры.НалогНаПрибыльДляДивидендов = 0;
			КонецЕсли;
...
Функция ВыводитьФорму2НДФЛ2018Года(НалоговыйПериод, ДатаДокумента) 

	Возврат НалоговыйПериод >= 2017 И ДатаДокумента >= '20180212';

КонецФункции 

Функция ВыводитьФорму2НДФЛ2019Года(НалоговыйПериод, ДатаДокумента)
...
Функция ВыводитьФорму2НДФЛ2021Года(НалоговыйПериод, ДатаДокумента)
...
Функция ВыводитьФорму2НДФЛ2022Года(НалоговыйПериод, ДатаДокумента)
...
Функция ВыводитьФорму2НДФЛ2024Года(НалоговыйПериод, ДатаДокумента)

	ДатаНачалаДействия = '20240101';
	Возврат НалоговыйПериод > Год(ДатаНачалаДействия) - 1 И ДатаДокумента >= ДатаНачалаДействия;
