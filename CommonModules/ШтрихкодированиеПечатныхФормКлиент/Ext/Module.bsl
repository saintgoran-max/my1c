...
Процедура ОбъектНеНайден(Штрихкод, ИскатьПоВсемПоддерживаемымОбъектам = Истина) Экспорт
	...
	Если СтрокаПохожаНаШтрихкодПечатнойФормы(Штрихкод) Тогда
		
		АктивнаФормаРМК = Истина;
		ПараметрыОткрытия = Новый Структура("ПроверкаОткрытойформы", Истина);
		ФормаРМК = ПолучитьФорму("Обработка.РабочееМестоКассира.Форма.ФормаРМК", ПараметрыОткрытия);
		Если ФормаРМК = Неопределено Тогда
			АктивнаФормаРМК = Ложь;
		ИначеЕсли Не ФормаРМК.Открыта() Или Не ФормаРМК.ВводДоступен() Тогда
			АктивнаФормаРМК = Ложь;
		Иначе
			АктивнаФормаРМК = Истина;
		КонецЕсли;
		
		Если АктивнаФормаРМК Тогда
			Возврат;
		КонецЕсли;
