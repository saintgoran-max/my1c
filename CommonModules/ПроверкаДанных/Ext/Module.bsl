...
Процедура ВыполнитьПроверкуСведенийПоОрганизации(ДеревоРезультата, Организация, ДатаНачала, ДатаОкончания,
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если ОграниченияПроверки.Найти("РегистрационныйНомерПФР_ИП") <> Неопределено Тогда
			ПроверитьРегистрационныйНомерПФР(Организация, Истина, ПоляСОшибками);
		КонецЕсли;
	...
	Если ОграниченияПроверки.Найти("РегистрационныйНомерПФР") <> Неопределено Тогда
		ПроверитьРегистрационныйНомерПФР(Организация, Ложь, ПоляСОшибками);
	КонецЕсли;
...
Процедура ПроверитьРегистрационныйНомерПФР(Организация, НаСебя, ПоляСОшибками)
	
	ИспользуетсяНомерСФР = ЗарплатаКадры.ИспользованиеРегистрационногоНомераСФР(ТекущаяДатаСеанса(), Организация);
	ИмяРеквизитаНомерПФР = ?(НаСебя, "РегистрационныйНомерПФР_ИП", "РегистрационныйНомерПФР");
	РегНомерПФР = Организация[ИмяРеквизитаНомерПФР];
	РегНомерСФР = Организация.РегистрационныйНомерСФР;
	ПолеРегНомерПФР = СтрШаблон("Объект.%1", ИмяРеквизитаНомерПФР);
	
	Если ПустаяСтрока(РегНомерПФР) И ПустаяСтрока(РегНомерСФР) Тогда
		Если ИспользуетсяНомерСФР Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерСФР", НСтр("ru='Не указан Регистрационный номер СФР'"));
		Иначе
			ТекстОшибки = ?(НаСебя,
				НСтр("ru='Не указан Регистрационный номер в ПФР в качестве работодателя'"),
				НСтр("ru='Не указан Регистрационный номер в ПФР (на себя)'"));
			ПоляСОшибками.Вставить(ПолеРегНомерПФР, ТекстОшибки);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	
	Если ИспользуетсяНомерСФР И ЗначениеЗаполнено(РегНомерСФР) Тогда
		Если НЕ ПерсонифицированныйУчет.ПроверитьРегистрационныйНомерПФР(Организация.РегистрационныйНомерСФР) Тогда
			ПоляСОшибками.Вставить("Объект.РегистрационныйНомерСФР",НСтр("ru='Некорректное значение Регистрационного номера в СФР'"));
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если НаСебя Тогда
		Если СтрДлина(СокрЛП(РегНомерПФР)) <> 14 Тогда
			ПоляСОшибками.Вставить(ПолеРегНомерПФР,	НСтр("ru='Некорректное значение Регистрационного номера в ПФР (на себя)'"));
		КонецЕсли;
	Иначе
		Если НЕ ПроверкаДанныхКлиентСервер.РегНомерПФРСоответствуетТребованиям(СокрЛП(РегНомерПФР)) Тогда
			ПоляСОшибками.Вставить(ПолеРегНомерПФР,
