...
//		* ДатаОбработки - Дата - дата документа, текущая дата если не заполнено
//		* НалогообложениеНДС -  ПеречислениеСсылка.ТипыНалогообложенияНДС - тип налогообложения НДС объекта
//		* ЕстьПартииСНДС - Булево - наличие в объекте партий номенклатуры с НДС. Если отсутствует - Ложь
//		* Номенклатура - СправочникСсылка.Номенклатура
//		* НоменклатураВидСтавкиНДС - ПеречислениеСсылка.ВидыСтавокНДС - вид ставки НДС номенклатуры
//		* Партия - СправочникСсылка.ПартииНоменклатуры
//		* ВидСтавкиНДСПоУмолчанию - ПеречислениеСсылка.ВидыСтавокНДС
//
Функция ПараметрыОпределенияСтавкиНДС(СтруктураДанные = Неопределено) Экспорт
	
	ПараметрыНДС = Новый Структура;
	ПараметрыНДС.Вставить("Организация");
	ПараметрыНДС.Вставить("ДатаОбработки");
	ПараметрыНДС.Вставить("НалогообложениеНДС");
	ПараметрыНДС.Вставить("ЕстьПартииСНДС", Ложь);
	ПараметрыНДС.Вставить("Номенклатура");
	ПараметрыНДС.Вставить("НоменклатураВидСтавкиНДС");
	ПараметрыНДС.Вставить("Партия");
	ПараметрыНДС.Вставить("ВидСтавкиНДСПоУмолчанию");
...
Функция ДанныеНДССтрокиНоменклатуры(СтруктураДанныеСтроки) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(НоменклатураВидСтавки) И ЗначениеЗаполнено(СтруктураДанные.Номенклатура) Тогда
		НоменклатураВидСтавки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Номенклатура, "ВидСтавкиНДС");
	КонецЕсли;
	
	ВидСтавкиПоУмолчанию = СтруктураДанные.ВидСтавкиНДСПоУмолчанию;
	Если НЕ ЗначениеЗаполнено(ВидСтавкиПоУмолчанию) Тогда
		ВидСтавкиПоУмолчанию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураДанные.Организация, "ВидСтавкиНДСПоУмолчанию");
	КонецЕсли;
	
	ТипОблагаетсяНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС");
	...
	Если ЗначениеЗаполнено(СтруктураДанные.Партия) И ПолучитьФункциональнуюОпцию("ИспользоватьПартии") Тогда
		...
		Если ДанныеПартии.Статус = ПредопределенноеЗначение("Перечисление.СтатусыПартий.ТоварыНаКомиссии") Тогда
			...
			Если ДанныеПартии.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДС") Тогда
				...
				Иначе
					ВидСтавкиНДС = ?(ЗначениеЗаполнено(НоменклатураВидСтавки), НоменклатураВидСтавки, ВидСтавкиПоУмолчанию);
	...
	Если НоменклатураВидСтавки = ВидСтавкиБезНДС ИЛИ НоменклатураВидСтавки = ВидСтавкиНулевая
...
Процедура ОбновитьСтавкуНДСТабличнойЧасти(Объект, ИмяТабличнойЧасти, УчитыватьПартии, ВидСтавкиНДСПоУмолчанию = Неопределено) Экспорт
	...
	ПропуститьЗаполнение = Новый Массив;
	Для Каждого Строка Из Объект[ИмяТабличнойЧасти] Цикл
		...
		Иначе
			ПропуститьЗаполнение.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		...
		Если ПропуститьЗаполнение.Найти(СтрокаТабличнойЧасти) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция СписокДокументовПоЗаказу(Объект, ТипОперации, СтруктураПараметров, ИмяТЧ = "Запасы", СостояниеВыполнения = Неопределено, ТипДокумента = Неопределено) Экспорт
	...
	Иначе
		КакВЗаказеПокупателя = ?(ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя")ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику"), Истина, Ложь);
	КонецЕсли;
	
	Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя") ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
