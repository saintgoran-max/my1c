...
Функция ЗапросСпискаПользователей(НастройкиТелефонии) Экспорт
	...
	ПараметрыЗапроса.Вставить("extension", Неопределено);
	
	ПараметрыЗвонкаJson = ОтправкаЗапросов.СоздатьJSONИзСтруктуры(ПараметрыЗапроса);
	ПодписьЗапроса = ПолучитьSign(НастройкиТелефонии.vpbx_api_key, ПараметрыЗвонкаJson, НастройкиТелефонии.vpbx_api_salt);
	
	ПараметрыОтправки = ОтправкаЗапросов.НовыйПараметрыОтправки();
	ПараметрыОтправки.Сервер = КорневойАдрес();
	ПараметрыОтправки.АдресРесурса = "config/users/request";
	ПараметрыОтправки.ПараметрыТела.Вставить("json", ПараметрыЗвонкаJson);
	ПараметрыОтправки.ПараметрыТела.Вставить("vpbx_api_key", НастройкиТелефонии.vpbx_api_key);
	ПараметрыОтправки.ПараметрыТела.Вставить("sign", ПодписьЗапроса);
...
Процедура ОбработатьОтветЗапросаСпискаПользователей(HttpОтвет, НастройкиТелефонии, Результат) Экспорт
	...
	Иначе
		...
		Если ПараметрыОтвета.Свойство("users") И ТипЗнч(ПараметрыОтвета.users) = Тип("Массив") Тогда
			Для каждого Пользователь Из ПараметрыОтвета.users Цикл
				Если Пользователь.Свойство("telephony")
					И Пользователь.telephony.Свойство("extension")
					И Пользователь.telephony.extension <> Неопределено
					И Пользователь.Свойство("general")
					И Пользователь.general.Свойство("name") Тогда
					
					ПользовательАТС = Новый Структура("ИмяПользователя, ВнутреннийНомер");
					ПользовательАТС.ИмяПользователя = Пользователь.general.name;
					ПользовательАТС.ВнутреннийНомер = Пользователь.telephony.extension;
...
Функция ПроверкаПолейАвторизацииОАТС(ТипГруппыПроверок) Экспорт
	...
	ТелефонияДиагностика.ПроверитьОбязательноеПоле(
		...
		ТипГруппыПроверок,
		НСтр("ru='Уникальный код АТС'"),
		НастройкиТелефонии.vpbx_api_key);
		
	ТелефонияДиагностика.ПроверитьОбязательноеПоле(
		РезультатыТекущейПроверки,
		ТипГруппыПроверок,
		НСтр("ru='Ключ для создания подписи'"),
