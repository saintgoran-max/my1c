...
// Формирует таблицу номенклатуры для экспорта после формирования отчета
//
// Параметры:
//  ФормаОтчета	 - 	ФормаКлиентскогоПриложения - форма отчета
//
Процедура СформироватьНоменклатуруДляЭкспортаПослеФормированияОтчета(ФормаОтчета) Экспорт
	
	Если НЕ ЭтоАдресВременногоХранилища(ФормаОтчета.ОтчетДанныеРасшифровки) Тогда
		Возврат;
	КонецЕсли; 
	
	ФормаОтчета.НоменклатураОтчета.Очистить();
	МассивНоменклатуры = Новый Массив;
	ОтчетыУНФВызовСервера.СформироватьНоменклатуруДляЭкспорта(ФормаОтчета.ОтчетДанныеРасшифровки, МассивНоменклатуры);
		
	Для Каждого Номенклатура Из МассивНоменклатуры Цикл
		НоваяНоменклатура = ФормаОтчета.НоменклатураОтчета.Добавить();
		НоваяНоменклатура.Номенклатура = Номенклатура;
	КонецЦикла;
