...
Процедура ПроверкаГотовностиИсточниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ФормированиеЭлектронногоДокументаЗавершение = Новый ОписаниеОповещения(
		"ФормированиеЭлектронногоДокументаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	...
	ПараметрыДействия.ОбъектыДействий.ОбъектыУчета.Добавить(ДополнительныеПараметры.ОбъектУчета);
	
	ИнтерфейсДокументовЭДОКлиент.НачатьВыполнениеДействийПоЭДО(
		ФормированиеЭлектронногоДокументаЗавершение,
		ПараметрыДействия);
	
КонецПроцедуры

Процедура ФормированиеЭлектронногоДокументаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Итог.ОбработанныеДокументы.Количество() = 0 Тогда
		...
		ПараметрыОповещения.Вставить(
			...
			Новый ОписаниеОповещения(
				"ОбработкаОшибокФормированияЭДОУспешноеЗавершение", ЭтотОбъект, ДополнительныеПараметры));
		...
		Если Результат.Свойство("ОшибкиФормирования") И Результат.ОшибкиФормирования.Количество() Тогда
			...
			ИнтерфейсДокументовЭДОКлиент.НачатьОбработкуОшибокФормированияДокумента(
				Оповещение, Результат.ОшибкиФормирования, ПараметрыОповещения);
			Возврат;
		...
		Иначе
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Не удалось сформировать электронный документ по объекту учета: %1'"), ДополнительныеПараметры.ОбъектУчета);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
		КонецЕсли;
		
	Иначе
		
		Для Каждого КлючИЗначение Из Результат.Итог.ОбработанныеДокументы Цикл
			
			ПараметрыСобытия = ИнтерфейсДокументовЭДОКлиент.НовыеПараметрыСобытияОбновленияСостоянияЭДО();
			ПараметрыСобытия.ЭлектронныеДокументы.Добавить(КлючИЗначение.Ключ);
...
Процедура ОбработкаОшибокФормированияЭДОУспешноеЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ВыполнитьДействияПоЭДОПослеОбработкиОшибокФормирования(Результат, ПараметрыОповещения) Экспорт
	...
	ОповещениеЗавершения = Новый ОписаниеОповещения(
		"ФормированиеЭлектронногоДокументаЗавершение", ЭтотОбъект, ПараметрыОповещения);
	
	Если Результат.ОшибкаНастройкиСОднимКлючем Тогда
		ИнтерфейсДокументовЭДОКлиент.НачатьВыполнениеДействийПоЭДО(ОповещениеЗавершения, ПараметрыОповещения.ПараметрыВыполненияДействийПоЭДО);
	Иначе
		ИнтерфейсДокументовЭДОКлиент.НачатьВыполнениеДействийПоЭДО(ОповещениеЗавершения, Результат.ПараметрыВыполненияДействийПоЭДО);
	КонецЕсли;
