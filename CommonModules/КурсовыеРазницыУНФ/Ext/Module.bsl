...
Функция ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПоставщиками(МенеджерВременныхТаблиц, СЗачетомАванса, НомерЗапроса, ПолучатьРеквизитыРасчетов = Ложь) Экспорт
	...
	Иначе
		...
		|	ТаблицаРасчеты.ТипРасчетов КАК ТипРасчетов,
		|	ВЫБОР
		|		КОГДА ТаблицаРасчеты.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыРасчетов.Аванс)
		|			ТОГДА ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * ТаблицаРасчеты.Курс * КурсыВалютУчетаСрезПоследних.Кратность / (КурсыВалютУчетаСрезПоследних.Курс * ТаблицаРасчеты.Кратность) - ЕСТЬNULL(ТаблицаОстатки.СуммаОстаток, 0)
		|		ИНАЧЕ ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * КурсыВалютУчетаСрезПоследних.Курс / КурсыВалютУчетаСрезПоследних.Кратность - ЕСТЬNULL(ТаблицаОстатки.СуммаОстаток, 0)
		|	КОНЕЦ КАК СуммаКурсовойРазницы,
		|	ЕСТЬNULL(ТаблицаОстатки.СуммаВалОстаток, 0) * КурсыВалютУчетаСрезПоследних.Курс / КурсыВалютУчетаСрезПоследних.Кратность - ЕСТЬNULL(ТаблицаОстатки.СуммаРегОстаток, 0) КАК СуммаКурсовойРазницыРег,
		|	ТаблицаРасчеты.Валюта КАК Валюта,
		|	ТаблицаРасчеты.СчетУчета КАК СчетУчета
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсовыхРазницРасчетыСПоставщиками
		|ИЗ
