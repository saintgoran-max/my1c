...
Процедура СформироватьВТРасширенныхСведенийОДоходах(НачалоПериода, ОкончаниеПериода, Организация, МенеджерВременныхТаблиц, ЕстьОтборПоФизическимЛицам = Ложь, ИсключатьДоходыРегистратора = Ложь, Регистратор = Неопределено) Экспорт
	...
	Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
		...
		ТаблицаПовышающихКоэффициентов.Колонки.Добавить("РасчетныйПериод", Новый ОписаниеТипов("Дата", , , Новый КвалификаторыДаты()));	
		ТаблицаПовышающихКоэффициентов.Колонки.Добавить("ПовышающийКоэффициент", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла()));	
		...
		|	МинимальнаяОплатаТрудаРФСрезПоследних.Значение * ТаблицаПовышающихКоэффициентов.ПовышающийКоэффициент КАК Значение
...
Функция НоваяСтрокаПовышающегоКоэффициента(ТаблицаЗначений, РасчетныйПериод)
	
	НоваяСтрока = ТаблицаЗначений.Добавить();
	НоваяСтрока.РасчетныйПериод = РасчетныйПериод;
	НоваяСтрока.ПовышающийКоэффициент = ?(РасчетныйПериод < '20250101', 1, 1.5);
	
КонецФункции

Процедура ВыделитьМРОТ(СтрокаМесяца, Показатель, СтрокиДеления)
...
Процедура ВыделитьБазуМРОТНаОсновнойТариф(СтрокаМесяца, Результат, ОсновнойТариф)
	...
	Если ТребуетсяВыделятьМРОТ_ПФР Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаПФР", СтрокиДеления);
		
		СтрокаИсходный.СуммаПревысившаяПределПФР = Мин(СтрокаИсходный.БазаПФР, СтрокаМесяца.СуммаПревысившаяПределПФР);
		СтрокаОсновной.СуммаПревысившаяПределПФР = СтрокаМесяца.СуммаПревысившаяПределПФР - СтрокаИсходный.СуммаПревысившаяПределПФР;
	Иначе
	...
	Если ТребуетсяВыделятьМРОТ_ФСС Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаФСС", СтрокиДеления);

		СтрокаИсходный.СуммаПревысившаяПределФСС = Мин(СтрокаИсходный.БазаФСС, СтрокаМесяца.СуммаПревысившаяПределФСС);
		СтрокаОсновной.СуммаПревысившаяПределФСС = СтрокаМесяца.СуммаПревысившаяПределФСС - СтрокаИсходный.СуммаПревысившаяПределФСС;
	Иначе
	...
	Если ТребуетсяВыделятьМРОТ_ФОМС Тогда
		ВыделитьМРОТ(СтрокаМесяца, "БазаФОМС", СтрокиДеления);
		
		СтрокаИсходный.СуммаПревысившаяПределФОМС = Мин(СтрокаИсходный.БазаФОМС, СтрокаМесяца.СуммаПревысившаяПределФОМС);
		СтрокаОсновной.СуммаПревысившаяПределФОМС = СтрокаМесяца.СуммаПревысившаяПределФОМС - СтрокаИсходный.СуммаПревысившаяПределФОМС;
	Иначе
