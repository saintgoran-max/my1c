...
Процедура УстановитьУсловноеОформлениеПоЦветамСостояний(УсловноеОформлениеКД,
	...
	УчитыватьЦветФона = ПолноеИмяСправочникаСостояний = "Справочник.СостоянияЗаказовПокупателей" Или ПолноеИмяСправочникаСостояний = "Справочник.СостоянияЗаказНарядов";		
	...
	Пока ВыборкаСостояний.Следующий() Цикл
		...
		Если УчитыватьЦветФона Тогда
			
			ЦветСостояния	  = ВыборкаСостояний.Цвет.Получить();
			ЦветФонаСостояния = ВыборкаСостояний.ЦветФона.Получить();
			
			Если ТипЗнч(ЦветСостояния) <> Тип("Цвет") И ТипЗнч(ЦветФонаСостояния) <> Тип("Цвет") Тогда
				Продолжить;
			КонецЕсли;   
		...
		// TODO: так как механизм универсальный для разных справочников состояний, необходимо подумать над универсальносью настройки цвета фона
		...
		Если УчитыватьЦветФона Тогда  
			...
			ЭлементУсловногоОформленияФон = УсловноеОформлениеКД.Элементы.Добавить();
			
			ЭлементУсловногоОформленияФон.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветФонаСостояния); 
			ЭлементУсловногоОформленияФон.ИдентификаторПользовательскойНастройки = "ЦветФонаСостояния";
			ЭлементУсловногоОформленияФон.РежимОтображения	= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
			ЭлементУсловногоОформленияФон.Представление	= НСтр("ru='Оформление фона в цвет состояния'");  
			
			ЭлементОтбора = ЭлементУсловногоОформленияФон.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбора.ЛевоеЗначение		= Новый ПолеКомпоновкиДанных(ИмяПоляСостояния);
			ЭлементОтбора.ВидСравнения		= ВидСравненияКомпоновкиДанных.Равно;
			ЭлементОтбора.ПравоеЗначение	= ВыборкаСостояний.Ссылка;
		
			Если ОформляемоеПоле <> Неопределено Тогда
				ОформляемоеПолеКД = ЭлементУсловногоОформленияФон.Поля.Элементы.Добавить();
				ОформляемоеПолеКД.Поле = Новый ПолеКомпоновкиДанных(ОформляемоеПоле);
				ОформляемоеПолеКД.Использование = Истина;
			КонецЕсли;
...
// Заполняет дополнительные свойства перед записью Заказа
// Параметры:
//  ЗаказОбъект - ДокументОбъект.ЗаказНаПеремещение, ДокументОбъект.ЗаказНаПроизводство, ДокументОбъект.ЗаказПокупателя, ДокументОбъект.ЗаказПоставщику 
//  РежимЗаписи - РежимЗаписиДокумента
//
Процедура ПередЗаписьюЗаказа(ЗаказОбъект, РежимЗаписи = Неопределено) Экспорт
	...
	ИзмениласьПроведенность = Ложь;
	ИзмениласьПометкаУдаления  = Ложь;
	...
	Если Не ЗаказОбъект.ЭтоНовый() Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаказОбъект.Ссылка, "Проведен, ПометкаУдаления");
		ИзмениласьПометкаУдаления = ЗначенияРеквизитов.ПометкаУдаления <> ЗаказОбъект.ПометкаУдаления ;   
		Если ТипЗнч(РежимЗаписи) = Тип("РежимЗаписиДокумента") Тогда
			ПредыдущаяПроведенность = ЗначенияРеквизитов.Проведен; 
			...
			ИзмениласьПроведенность = (ЗаказОбъект.Проведен = Ложь И ЭтоПроведение) Или (ЗаказОбъект.Проведен = Истина И ЭтоОтменаПроведения);
		КонецЕсли;
	...
	ДанныеИсторииСостояния.Вставить("ИзмениласьПроведенностьПометкаУдаления", ИзмениласьПроведенность Или ИзмениласьПометкаУдаления);
...
Процедура ПриЗаписиЗаказа(ЗаказОбъект) Экспорт
	...
	Если ЗаказОбъект.ДополнительныеСвойства.Свойство("ДанныеИсторииСостояния") Тогда
		...
		ИначеЕсли ЗаказОбъект.ДополнительныеСвойства.ДанныеИсторииСостояния.ИзмениласьПроведенностьПометкаУдаления Тогда
			
			АссистентУправленияПриСрабатыванииСобытия(ЗаказОбъект.Ссылка, ЗаказОбъект.ДополнительныеСвойства.ДанныеИсторииСостояния);
...
// Состояние "Не обработан" заказов покупателя и заказ-нарядов
// 
// Возвращаемое значение:
//  Строка
//
Функция СостояниеНеОбработан() Экспорт
	
	Возврат НСтр("ru = 'Не обработан'");
	
КонецФункции

// Состояние "В работе" заказов покупателя и заказ-нарядов
// 
// Возвращаемое значение:
//  Строка
//
Функция СостояниеВРаботе() Экспорт
	
	Возврат НСтр("ru = 'В работе'");
	
КонецФункции

// Состояние "Выставлен счет" заказов покупателя и заказ-нарядов
// 
// Возвращаемое значение:
//  Строка
//
Функция СостояниеВыставленСчет() Экспорт
	
	Возврат НСтр("ru = 'Выставлен счет'");
	
КонецФункции

// Состояние "На выполнении" заказов покупателя и заказ-нарядов
// 
// Возвращаемое значение:
//  Строка
//
Функция СостояниеНаВыполнении() Экспорт
	
	Возврат НСтр("ru = 'На выполнении'");
	
КонецФункции

// Состояние "Завершен" заказов покупателя и заказ-нарядов
// 
// Возвращаемое значение:
//  Строка
//
Функция СостояниеЗавершен() Экспорт
	
	Возврат НСтр("ru = 'Завершен'");
...
Процедура АссистентУправленияСинхронизироватьСтатусЗаказПокупателя(Предмет, ЗначенияЗаполнения, ДополнительныеПараметры, Результат) Экспорт
	...
	Для каждого ЗаказПокупателя Из ЗаказыПокупателей Цикл
		...
		ОтобратьПроведенныеНеПомеченныеЗаказы(ПодчиненныеЗаказы);	
...
Процедура АссистентУправленияСинхронизироватьСтатусПодчиненныхЗаказовЗаказаПокупателя(Предмет, ЗначенияЗаполнения, ДополнительныеПараметры, Результат) Экспорт
	...
	Для каждого ПодчиненныйЗаказ Из ПодчиненныеЗаказы Цикл
        ...
        ОтобратьПроведенныеНеПомеченныеЗаказы(ОбеспечиваемыеЗаказы);	
...
Процедура ОтобратьПроведенныеНеПомеченныеЗаказы(МассивЗаказов)
	
	МассивКорректныхЗаказов = Новый Массив;
	Для Каждого Заказ Из МассивЗаказов Цикл
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заказ.Ссылка, "Проведен, ПометкаУдаления");
		Если ЗначенияРеквизитов.Проведен = Истина И ЗначенияРеквизитов.ПометкаУдаления = Ложь Тогда
			МассивКорректныхЗаказов.Добавить(Заказ);	
		КонецЕсли;
	КонецЦикла;  
	
	МассивЗаказов = МассивКорректныхЗаказов;
