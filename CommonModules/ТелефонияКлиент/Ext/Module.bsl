...
Процедура ОбработчикНовогоСообщенияОбсуждениеЖурналЗвонков(Сообщение, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ДанныеОповещения.ТипЗвонка, "Диагностика") Тогда
		
		Если ДанныеОповещения.Свойство("УникальныйИдентификаторПроверки") Тогда
			УникальныйИдентификаторПроверки = ДанныеОповещения.УникальныйИдентификаторПроверки;
			Оповестить("Телефония_ДиагностикаПодключения", УникальныйИдентификаторПроверки);
		КонецЕсли;
...
Процедура ОбработкаВходящегоЗвонка(ДанныеЗвонка = Неопределено) Экспорт
	...
	Если ДанныеЗвонка.ВходящееИсходящее = ПредопределенноеЗначение("Перечисление.ВходящееИсходящееСобытие.Входящее") Тогда
		...
		КонецЕсли;
	ИначеЕсли ДанныеЗвонка.ВходящееИсходящее = ПредопределенноеЗначение("Перечисление.ВходящееИсходящееСобытие.Исходящее") Тогда
...
#Иначе
	...
	Если (Действие = "ОткрытьСобытие" ИЛИ Действие = "ОткрытьСобытиеИЗвонок") И Событие = Неопределено Тогда
		...
		Если Действие = "ОткрытьСобытиеИЗвонок" Тогда
			ЗначенияЗаполнения.Вставить("НабратьНомер",    Истина);
		КонецЕсли;
