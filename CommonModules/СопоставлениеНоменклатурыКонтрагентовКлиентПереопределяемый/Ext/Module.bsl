...
Процедура ОткрытьФормуНоменклатуры(Знач Параметры, Знач Владелец, Знач Уникальность, Знач ОповещениеОЗакрытии) Экспорт
	...
	Если Параметры.Свойство("ДополнительныеПараметры")
		И Параметры.ДополнительныеПараметры.Свойство("СопоставлениеНоменклатурыКонтрагентов")
		И Параметры.ДополнительныеПараметры.СопоставлениеНоменклатурыКонтрагентов.Свойство("НоменклатураКонтрагента") Тогда
		
		СтруктураДанных = Параметры.ДополнительныеПараметры.СопоставлениеНоменклатурыКонтрагентов.НоменклатураКонтрагента;
			
		ШтрихКодДляУстановкиПослеЗаписи = "";
		Если СтруктураДанных.Свойство("ШтрихкодыНоменклатуры") И ЗначениеЗаполнено(СтруктураДанных.ШтрихкодыНоменклатуры) Тогда
			ШтрихКодДляУстановкиПослеЗаписи = СокрЛП(СтруктураДанных.ШтрихкодыНоменклатуры);
		КонецЕсли;
			
		Если СтруктураДанных.Свойство("ШтрихкодКомбинации") И ЗначениеЗаполнено(СтруктураДанных.ШтрихкодКомбинации) Тогда
			Если Найти(ШтрихКодДляУстановкиПослеЗаписи, СтруктураДанных.ШтрихкодКомбинации) = 0 Тогда
				ШтрихКодДляУстановкиПослеЗаписи = СокрЛП(СтруктураДанных.ШтрихкодКомбинации) 
					+ ?(СтрДлина(ШтрихКодДляУстановкиПослеЗаписи) > 0, ",", "") + ШтрихКодДляУстановкиПослеЗаписи;
			КонецЕсли;
		КонецЕсли;
		
		Параметры.Вставить("ШтрихКодДляУстановкиПослеЗаписи", СокрЛП(ШтрихКодДляУстановкиПослеЗаписи));
