...
Процедура ЗаполнитьРезультатНачисления(Объект, Начисления) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПерваяПоловинаМесяца") Тогда
		Если Объект.ПерваяПоловинаМесяца Тогда
			ОкончаниеПериода = РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатаРасчетаАванса(Объект.МесяцНачисления);
	...
	Для каждого СтрокаВытеснения Из ТаблицаВытеснений Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПерваяПоловинаМесяца") Тогда
			Если Объект.ПерваяПоловинаМесяца Тогда
				ДатаОкончания = Мин(ДатаОкончания, РасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатаРасчетаАванса(Объект.МесяцНачисления));
			КонецЕсли;
		КонецЕсли;
...
Процедура СоздатьВТНачисленияЗарплаты(МенеджерВременныхТаблиц, ИмяВТНачисленияЗарплаты = "ВТНачисленияЗарплаты") Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(Начисления.ОсновнойВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени, ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ПустаяСсылка)) КАК КатегорияОсновногоНачисления
		|ПОМЕСТИТЬ ВТНачисленияОтбор
		|ИЗ
		...
		|			И (НачисленияУдержанияПоСотрудникам.ГруппаНачисленияУдержанияВыплаты = ЗНАЧЕНИЕ(Перечисление.ГруппыНачисленияУдержанияВыплаты.Начислено))
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияОтбор КАК ВТНачисленияОтбор
		|		ПО НачисленияУдержанияПоСотрудникам.НачислениеУдержание = ВТНачисленияОтбор.Ссылка
		|			И (НЕ ВТНачисленияОтбор.КатегорияОсновногоНачисления В (&СписокКатегорий))
		|
		|СГРУППИРОВАТЬ ПО
...
Функция СоздатьВТНачисленияПоОтсутствиям(МенеджерВременныхТаблиц, ТолькоРазрешенные, МесяцНачисления, Организация, Подразделение = Неопределено, СписокСотрудников = Неопределено, ИсключаемыйРегистратор = Неопределено, ИмяВТНачисленияПоОтсутствиям = "ВТНачисленияПоОтсутствиям")
	...
	Запрос.Текст =
		...
		|						ИЛИ ВЫРАЗИТЬ(НачисленияУдержанияПоСотрудникам.НачислениеУдержание КАК ПланВидовРасчета.Начисления).ОсновнойВидРасчета.КатегорияНачисленияИлиНеоплаченногоВремени В (&СписокКатегорий))
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("62c4ff72-3819-4803-83c4-b1217d6033e9");
	Обработчик.Комментарий = НСтр("ru = 'Создание начислений долей РК и СН среднего заработка.'");
