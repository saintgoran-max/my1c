...
Процедура ПриОпределенииВозможностиЗаменыЭлементов(Знач ИмяОбъектаМетаданных, Знач ПарыЗамен, Знач ПараметрыЗамены, НедопустимыеЗамены) Экспорт
	...
	Если ИмяОбъектаМетаданных = "Справочник.Лиды" Тогда
		
		Лиды = Новый Массив;
		Для Каждого ПараЛидов Из ПарыЗамен Цикл
			Лиды.Добавить(ПараЛидов.Ключ);
			Лиды.Добавить(ПараЛидов.Значение);
		КонецЦикла;
		
		РеквизитыЛидов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(Лиды, "Вид,ВариантЗавершения,Представление");
		
		Для Каждого РеквизитыЛида Из РеквизитыЛидов Цикл
			
			ДанныеЛида = РеквизитыЛида.Значение;
			
			Если ДанныеЛида.ВариантЗавершения = Перечисления.ВариантЗавершенияРаботыСЛидом.ПереведенВПокупателя
				И ДанныеЛида.Вид <> Перечисления.ВидыЛидов.ПовторноеОбращение Тогда
				
				НедопустимыеЗамены[РеквизитыЛида.Ключ] = СтрШаблон(
				НСтр("ru = 'Лид ""%1"" уже является качественным.'"), ДанныеЛида.Представление);
		...
		Для Каждого РеквизитыЛида Из РеквизитыЛидов Цикл
			...
			Если ДанныеЛида.Вид = Перечисления.ВидыЛидов.ПовторноеОбращение
				И ДанныеЛида.ВариантЗавершения <> Перечисления.ВариантЗавершенияРаботыСЛидом.ПереведенВПокупателя Тогда
				
				НедопустимыеЗамены[РеквизитыЛида.Ключ] = СтрШаблон(
				НСтр("ru = 'Отвяжите лида ""%1"" от покупателя.'"), ДанныеЛида.Представление);
