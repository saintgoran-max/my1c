...
#Область ПодготовкаОбщихДанных
	...
	ОрганизацияНеПлательщикНДС = УчетнаяПолитика.СистемаНалогообложения(
		ПараметрыОтчета.Организация, ПараметрыОтчета.мДатаКонцаПериодаОтчета).СистемаНалогообложения 
	...
	Если Контейнер.Свойство("Раздел3_Прил1") Тогда
		...
		Если КонецДня(ПараметрыОтчета.мДатаКонцаПериодаОтчета) = КонецГода(ПараметрыОтчета.мДатаКонцаПериодаОтчета) Тогда
			...
			Для Каждого Показатель Из СтраницаРаздел3_Прил1 Цикл
				СтраницаРаздел3_Прил1[Показатель.Ключ] = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(
					...
					Если НЕ ПерваяСтраница Тогда
						...
						Для Каждого Показатель Из СтраницаРаздел3_Прил1 Цикл
							СтруктураНовойТаблицы.Вставить(Показатель.Ключ, 
								РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа(ТипЗнч(Показатель.Значение)));
						КонецЦикла;
...
#Область Разделы4и6
	...
	Если Контейнер.Свойство("Раздел4") Тогда 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Раздел4Начисления.П000400001003_1 КАК П000400001003_1,
		|	ВТ_Раздел4Начисления.П0004М1001003_1 КАК П0004М1001003_1,
		|	ВТ_Раздел4Начисления.П000400001003 КАК П000400001003,
		|	ВТ_Раздел4Начисления.П0004М1001003 КАК П0004М1001003,
		|	ВТ_Раздел4Начисления.П000400002003_1 КАК П000400002003_1,
		|	ВТ_Раздел4Начисления.П0004М1002003_1 КАК П0004М1002003_1,
		|	ВТ_Раздел4Начисления.П000400002003 КАК П000400002003,
		|	ВТ_Раздел4Начисления.П0004М1002003 КАК П0004М1002003
		|ИЗ
...
Функция СведенияОРуководителе(ВыгружаемыйДокумент) 	
	...
	ФизЛицо = ВыгружаемыйДокумент.Организация.ПодписьРуководителя.ФизическоеЛицо;
	ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ВыгружаемыйДокумент.Дата, Новый Структура("ФизическоеЛицо", ФизЛицо));
