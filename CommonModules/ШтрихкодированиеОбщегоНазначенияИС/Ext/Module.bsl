...
Функция ДанныеПоШтрихкодамEAN(ШтрихкодыEAN, ПараметрыСканирования = Неопределено, GTINПотребительскихУпаковок = Неопределено) Экспорт
	...
	Если Не ПараметрыПоискаРМК = Неопределено Тогда
		Если ПараметрыПоискаРМК.Свойство("ОрганизацияТовара") Тогда
			ПараметрыСканирования.Организация = ПараметрыПоискаРМК.ОрганизацияТовара;
			ПараметрыСканирования.Владелец = ПараметрыСканирования.Организация;
		КонецЕсли;
		Если ПараметрыПоискаРМК.Свойство("ККТТовара") Тогда
			ПараметрыСканирования.ККТФФД12ИСМП = ПараметрыПоискаРМК.ККТТовара;
		КонецЕсли;
	КонецЕсли;
