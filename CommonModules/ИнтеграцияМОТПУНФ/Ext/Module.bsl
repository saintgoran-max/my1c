...
Процедура ПриЗаполненииИнформацииПоШтрихкодамEAN(ДанныеПоШтрихкодамEAN, ПараметрыПоискаРМК = Неопределено) Экспорт
	...
	Если Не ПараметрыПоискаРМК = Неопределено Тогда
			...
			Если ДанныеШтрихкодаРМК.ЗначенияПоиска.Количество() > 0 Тогда
				Для Каждого ЗначениеПоиска Из ДанныеШтрихкодаРМК.ЗначенияПоиска Цикл
					Если ЗначениеПоиска.ТипОбъекта = Тип("СправочникСсылка.ШтрихкодыУпаковокТоваров")
						...
						КоличествоСтрокТаблицы = ДанныеПоШтрихкодамРезультат.Количество();
						Для НомерСтрокиТаблицы = 1 По КоличествоСтрокТаблицы - 1 Цикл
							ДанныеПоШтрихкодамРезультат.Удалить(КоличествоСтрокТаблицы-НомерСтрокиТаблицы);
						КонецЦикла;
		...
		Если НайденаНоменклатура И Не НайденоНесколькоПозиций Тогда
			...
			Если ЗначенияПоискаПоШК.Количество() = 1 Тогда
				...
				Если ЗначениеПоискаПоШК.Свойство("СтруктураНайденнойНоменклатуры") Тогда
					ОрганизацияТовара = ЗначениеПоискаПоШК.СтруктураНайденнойНоменклатуры.Организация;
					Если ЗначениеЗаполнено(ОрганизацияТовара) Тогда
						ПараметрыПоискаРМК.Вставить("ОрганизацияТовара", ОрганизацияТовара);
						Если ПараметрыПоискаРМК.Свойство("СоответствияККТОрганизаций") Тогда
							ККТТовара = ПараметрыПоискаРМК.СоответствияККТОрганизаций.Получить(ОрганизацияТовара);
							Если ЗначениеЗаполнено(ККТТовара) Тогда
								ПараметрыПоискаРМК.Вставить("ККТТовара", ККТТовара);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
