...
//    * ИмяФайлаВТранслите - Булево - если Истина, то имя файлп будет на латинице.
...
//    * Идентификатор - Строка - уникальный идентфикатор колонки.
//    * Представление - Строка - представление колонки пользователю (шапка табличной части).
//    * ПорядокОтображения - Число - Порядок отображения колонко пользователю, упорядочивание по возрастанию.
...
//         по наступлению указанного времени заказ будет анулирован.
...
//       ** ПокупательЗаголовок - Строка - представление покупателья (пример: Покупатель, Клиент).
...
//  СтатусЗаказа - Строка - Статус заказа, может принимать следующие зачения: "Оплачен", "ИстекСрокЖизни";
...
Процедура ПриЗагрузкеСтатусаЗаказа(
	...
	Если ОбязательныеПараметры.Получить("СБПc2b") <> Неопределено Тогда
		...
		СБПc2b 			   = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ОбязательныеПараметры.Получить("СБПc2b"));
		
		Если СБПc2b.РезультатОбработки.Свойство("СтатусОпераци") Тогда
			СБПc2b.РезультатОбработки.Вставить("СтатусОперации", СБПc2b.РезультатОбработки.СтатусОпераци);
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(РезультатОбработки, СБПc2b.РезультатОбработки);
...
// в форме формирования платежной ссылки онлайн заказоы.
//
// Параметры:
